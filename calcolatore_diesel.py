...
from core.path_generator import (
    isentropic_path, isochoric_path, isobaric_path, polytropic_path,
...
class DieselCAD(ctk.CTkFrame):
    ...
    def _compute(self):
        try:
            ...
            # 2->3 (isobaric heat addition, cutoff)
            P3_bar = P2_bar
            v3 = v2 * rc
            T3_K = T2_K * rc
            ...
            p1 = DieselPoint("1", T1_C, P1, v1, entropy(T1_K, P1), cp=cp, k=k, R=R)
            p2 = DieselPoint("2", T2_K-273.15, P2_bar, v2, entropy(T2_K, P2_bar), cp=cp, k=k, R=R)
            p2s = DieselPoint("2s", T2s_K-273.15, P1*(r**k), v2, p1.s, cp=cp, k=k, R=R)
            p3 = DieselPoint("3", T3_K-273.15, P3_bar, v3, entropy(T3_K, P3_bar), cp=cp, k=k, R=R)
            p4 = DieselPoint("4", T4_K-273.15, P4_bar, v4, entropy(T4_K, P4_bar), cp=cp, k=k, R=R)
            p4s = DieselPoint("4s", T4s_K-273.15, P3_bar*((v3/v1)**k), v4, p3.s, cp=cp, k=k, R=R)
    ...
    def _draw(self):
        ...
        for t in ["P-v", "T-s", "h-s"]:
            ...
            # Diesel real: 1→2 polytropic(comp), 2→3 isobaric, 3→4 polytropic(exp), 4→1 isochoric
            real_segments = [
            ...
            # Diesel ideal: 1→2s isentropic, 2s→3 isobaric, 3→4s isentropic, 4s→1 isochoric
            ideal_segments = [
                (p1,  p2s, "isentropic", "gray", "--", "Ideale (η=1)"),
                (p2s, p3,  "isobaric",   "gray", "--", ""),
    ...
    def _gen_path(pA, pB, process, n=80):
        """Generate path arrays between two DieselPoints using path_generator."""
