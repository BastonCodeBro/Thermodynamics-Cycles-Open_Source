...
const DieselPage = () => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 0) {
      ...
      const data = [addTrace(
        ...
        { name: 'Ciclo P-v', color: '#F87171', mode: 'lines+markers', markerSize: 10 }
      )];
  ...
  const diagramTabs = results ? [
    ...
    { id: 'schema', label: 'Schema', active: activeTab === 2, onClick: () => setActiveTab(2), content: <div ref={schematicRef}><SchematicDiagram type="diesel" accentColor={COLOR} /></div> },
  ] : null;
