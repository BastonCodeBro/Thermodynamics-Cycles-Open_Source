"""Ciclo Diesel — motore a accensione per compressione."""
import numpy as np
from core.thermo import GasPoint, K_AIR


def calc_diesel(P1_bar: float, T1_C: float, r: float, rc: float,
                k: float = 1.4, cv: float = 0.718, eta: float = 0.85):
    """
    Calcola il ciclo Diesel reale con rendimento isentropico.
    ...
    p3 = GasPoint("3", T3k - 273.15, P3, cp, k, R=R, h_ref=p1.h, s_ref=p1.s, **ref)
    ...
    results = {
        ...
        "eta": eta_cycle, "P_max": P3, "T_max": T3k - 273.15,
        "rc": rc,
    }
